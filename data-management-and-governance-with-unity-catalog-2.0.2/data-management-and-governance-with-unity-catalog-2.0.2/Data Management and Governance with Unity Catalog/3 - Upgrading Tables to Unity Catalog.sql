...
SELECT * 
FROM IDENTIFIER('hive_metastore.' || REPLACE(user_hive_schema, 'labuser', 'cat_') || '.movies')
LIMIT 10
...
DESCRIBE EXTENDED IDENTIFIER('hive_metastore.' || REPLACE(user_hive_schema, 'labuser', 'cat_') || '.movies')
...
-- MAGIC DEEP CLONE hive_metastore.{DA.user_hive_schema.replace('labuser','cat_')}.movies
-- MAGIC ''')
-- MAGIC
